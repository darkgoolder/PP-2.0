...
class S3SecretRepository:
    ...
    def __init__(
        self, storage, encryption, bucket: str = None, secrets_path: str = None
    ):
        ...
        logger.info(
            f"S3SecretRepository инициализирован: bucket={self.bucket}, path={self.secrets_path}"
        )
    ...
    async def _save_batch(self, batch: Dict[str, Any]) -> bool:
        ...
        return await self.storage.put_object(
            self.bucket, self.secrets_path, data.encode("utf-8")
        )
