...
class TrainModelUseCase:
    ...
    def _save_model(self, model, optimizer, epoch, val_acc, train_acc):
        """Сохранение модели (как во втором коде)"""
        # Создаём директорию, если её нет
        os.makedirs(os.path.dirname(self.model_save_path), exist_ok=True)

        # Сохраняем модель (формат как во втором коде)
        torch.save(
            {
                "epoch": epoch,
                "model_state_dict": model.state_dict(),
    ...
    def execute(self, config: Optional[TrainingConfig] = None) -> TrainingHistory:
        ...
        for epoch in range(config.num_epochs):
            ...
            # Сохраняем лучшую модель (как во втором коде)
            if val_acc > history.best_val_acc:
                self._save_model(model, optimizer, epoch, val_acc, train_acc)
        ...
        # ===== ВАЖНО: СОХРАНЯЕМ МОДЕЛЬ В КОНЦЕ ОБУЧЕНИЯ (ДАЖЕ ЕСЛИ ТОЧНОСТЬ НЕ УЛУЧШИЛАСЬ) =====
        # Это гарантирует, что модель всегда будет сохранена
        self._save_model(model, optimizer, epoch, val_acc, train_acc)
        logger.info("=" * 60)
        logger.info("🎉 ОБУЧЕНИЕ ЗАВЕРШЕНО!")
        logger.info("=" * 60)
