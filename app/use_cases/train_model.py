...
class RobustWagonDataset:
    ...
    def __init__(self, data_dir: str, class_names: List[str],
...
def create_efficientnet_model(num_classes: int = 3, pretrained: bool = True) -> nn.Module:
...
class TrainModelUseCase:
    ...
    def _create_dataloaders(self, config: TrainingConfig):
        ...
        train_dataset = RobustWagonDataset(
            self.data_dir, self.class_names,
            transform=get_train_transforms(), mode="train"
        )
        
        val_dataset = RobustWagonDataset(
            self.data_dir, self.class_names,
            transform=get_val_transforms(), mode="val"
        )
    ...
    def _train_epoch(self, model, loader, criterion, optimizer, device) -> Tuple[float, float]:
        """Обучение одной эпохи"""
        model.train()
        total_loss = 0.0
        correct = 0
