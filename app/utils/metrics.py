...
class MetricsMiddleware(BaseHTTPMiddleware):  # <-- ИСПРАВЛЕНО: наследуемся от BaseHTTPMiddleware
    """
    Middleware для сбора метрик HTTP запросов
    Автоматически собирает статистику по каждому запросу
    """
    
    async def dispatch(self, request: Request, call_next: Callable) -> Response:  # <-- ИСПРАВЛЕНО: dispatch вместо __call__
...
def record_prediction_metrics(class_name: str, confidence: float, device: str, duration: float):
    """
